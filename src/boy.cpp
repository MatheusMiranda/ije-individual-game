...
void Boy::on_collision(GameObject* other, Hitbox* p_my_hitbox, Hitbox* p_other_hitbox){
...
void Boy::on_event(GameEvent game_event){
  ...
  if(event_name == "JUMP"){
    ...
    if(animation_count2 == 5){
      moving_left_image->coordinatesOnTexture.first -= 108;
      animation_count2 = 0;
    }
    if(moving_left_image->coordinatesOnTexture.first <= 0)
      moving_left_image->coordinatesOnTexture.first = 756;
    ...
    if(animation_count == 5){
      moving_right_image->coordinatesOnTexture.first += 108;
      animation_count = 0;
    }

    if(moving_right_image->coordinatesOnTexture.first >= 864)
